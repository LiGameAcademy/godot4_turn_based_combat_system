...
func _process_effect(source: Node, target: Node, context : SkillExecutionContext) -> Dictionary:
	...
	if not is_instance_valid(source):
		push_error("ApplyStatusEffect: 无效的源节点引用")
		return {
			"success": false,
			"reason": "Invalid source node reference!"
		}
	if not is_instance_valid(target):
		push_error("ApplyStatusEffect: 无效的目标节点引用")
		return {
			"success": false,
	...
	if not is_instance_valid(target_skill_component):
		push_error("ApplyStatusEffect: 无效的目标技能组件引用")
		return {
			"success": false,
			"reason": "Invalid target skill component reference!"
		}

	var status_template_to_apply: SkillStatusData = status_to_apply.duplicate()
	...
	var application_result: Dictionary = target_skill_component.apply_status(status_template_to_apply, source, self)
	...
	if results.success and is_instance_valid(applied_status_instance):
		...
		# 播放状态效果成功应用的动画
		_request_visual_effect(&"status_applied_success", target, context.battle_manager, {"status_id": applied_status_instance.status_id, "status_name": applied_status_instance.status_name, "is_buff": applied_status_instance.status_type == SkillStatusData.StatusType.BUFF})
		if visual_effect != "": # 效果自定义的视觉
			_request_visual_effect(visual_effect, target, context.battle_manager, {"status_name": applied_status_instance.status_name})
		...
		_request_visual_effect(&"status_apply_failed_logic", target, context.battle_manager, {"status_id": status_template_to_apply.status_id, "reason": results.reason})
