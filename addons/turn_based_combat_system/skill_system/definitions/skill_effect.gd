...
func _get_determine_targets(caster: Node, targets: Array[Node], context : SkillExecutionContext) -> Array[Node]:
	...
	if target_override != "none":
		match target_override:
			...
			"all_allies":
				effect_targets = context.battle_manager.get_valid_ally_targets(true, caster) as Array[Node]
				effect_targets = context.battle_manager.get_valid_enemy_targets(caster) as Array[Node]
	
	return effect_targets

func _process_effect(source: Node, _target: Node, _context: SkillExecutionContext) -> Dictionary:
	await source.get_tree().create_timer(pre_cast_delay).timeout
...
## [param params] 视觉效果参数
## 发送视觉效果请求
func _request_visual_effect(effect_type: StringName, target: Node, params: Dictionary = {}) -> void:
