...
# 信号
signal skill_execution_started(caster: Node, skill: SkillData, targets: Array[Node])							## 技能执行开始信号	
signal skill_execution_completed(caster: Node, skill: SkillData, targets: Array[Node], results: Dictionary) 	## 技能执行完成信号 results 可以包含伤害、治疗、状态等信息
signal skill_failed(caster: Node, skill: SkillData, reason: String) 												## 技能失败信号 例如 MP不足, 目标无效等
...
## [return] 是否成功执行技能
func attempt_execute_skill(skill_data: SkillData, caster: Node, selected_targets: Array[Node], context : SkillExecutionContext) -> Dictionary:
...
## 尝试处理状态效果
func attempt_process_status_effects(effects : Array[SkillEffect], caster: Node, target: Node, context : SkillExecutionContext) -> Dictionary:
...
func trigger_game_event(event_source: Node, event_type: StringName, context: EventContext) -> void:
	# 打印事件日志（调试用）
...
## [return] 处理结果的字典
func _process_skill_effects_async(effects: Array[SkillEffect], caster: Node, targets: Array[Node], context: SkillExecutionContext) -> Dictionary:
	# 确保在操作前，所有参与者仍然有效
...
## 执行近战技能
func _process_melee_skill(skill_data: SkillData, caster: Node, targets: Array[Node], context: SkillExecutionContext) -> Dictionary:
...
## 执行远程技能
func _process_ranged_skill(skill_data: SkillData, caster: Node, targets: Array[Node], context: SkillExecutionContext) -> Dictionary:
	if skill_data.is_melee:
		return {
			"success": false,
...
## 私有方法：验证技能可用性
func _validate_skill_usability(skill: SkillData, caster: Node, targets: Array[Node], _context: SkillExecutionContext) -> Dictionary:
...
## [param targets] 目标列表
func _validate_skill_targets(skill: SkillData, targets: Array[Node]) -> bool:
...
## [param caster] 施法者
## [param skill] 要使用的技能数据
func _consume_skill_resources(caster: Node, skill: SkillData) -> void:
	if skill.mp_cost > 0:
