...
## 字典结构: {"attribute_template": SkillAttribute (拖拽.tres文件到这里), "override_base_value": float (可选, -1表示使用模板默认值)}
@export var attributes_to_initialize: Array[Dictionary] = []
...
func initialize_set() -> void:
    ...
    for attr_config_dict in attributes_to_initialize:
        var template: SkillAttribute = attr_config_dict.get("attribute_template")
        if not template is SkillAttribute:
            printerr("Invalid attribute template in attributes_to_initialize: ", template)
            continue
        
        # 关键：为每个角色复制独立的属性实例
        var attribute_instance: SkillAttribute = template.duplicate(true) as SkillAttribute
        if not attribute_instance:
            printerr("Failed to duplicate attribute template: ", template.resource_path)
            continue
            
        attribute_instance.set_owner_set(self) # 设置对父Set的引用
        
        var override_base = attr_config_dict.get("override_base_value", -1.0)
        if override_base != -1.0:
            attribute_instance.base_value = override_base # 覆盖模板中的默认基础值
    ...
    for attr_instance in _initialized_attributes.values():
        ...
        # 触发一次初始的recalculate，以防有零持续时间的自应用modifier或确保状态正确
        # 但通常初始时没有Modifier，所以这一步可能主要是为了钳制
        if attr_instance.recalculate_current_value(): # 即使没有modifier，也会执行钳制
            pass # 值可能因为钳制而改变
...
## 向指定属性应用一个Modifier
func apply_modifier(attribute_name: StringName, modifier: SkillAttributeModifier, source: Variant = null):
    var attr : SkillAttribute = get_attribute(attribute_name)
    ...
    set_current_value(attribute_name, proposed_new_current_value, source if source else modifier.source_id)

## 从指定属性移除一个Modifier (通过Modifier实例或其source_id)
func remove_modifier(attribute_name: StringName, modifier_or_id_to_remove: Variant, source: Variant = null):
    var attr : SkillAttribute = get_attribute(attribute_name)
    if not attr: return
    ...
    set_current_value(attribute_name, proposed_new_current_value, source)
